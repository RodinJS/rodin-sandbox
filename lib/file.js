...
export class File extends EventEmitter {
    ...
    load() {
        let url = this.url;
        const urlBeginning = this.url.substring(0, this.url.indexOf('/'));
        if (this.urlMap.hasOwnProperty(urlBeginning)) {
            url = path.join(this.urlMap[urlBeginning], this.url.substring(this.url.indexOf('/')));
        }
